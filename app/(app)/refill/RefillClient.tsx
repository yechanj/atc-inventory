...
export function RefillClient({
  initialRefillHistory,
  initialConsumeHistory,
}: {
  initialRefillHistory: HistoryRow[];
  initialConsumeHistory: HistoryRow[];
}) {
  ...
  const [editedPackageSize, setEditedPackageSize] = useState(1);
  const [saveAsRecommended, setSaveAsRecommended] = useState(false);
  const [saveAsPackageSize, setSaveAsPackageSize] = useState(false);
  ...
  const [refillHistory, setRefillHistory] = useState<HistoryRow[]>(initialRefillHistory);
  const [consumeHistory, setConsumeHistory] = useState<HistoryRow[]>(initialConsumeHistory);
  ...
  function selectCassette(c: Cassette) {
    ...
    setHighlightedIdx(-1);
    if (tab === "refill") {
      setMode(c.fullCapacity != null ? "direct" : c.packageSize > 1 ? "package" : "direct");
      setPackages(c.recommendedPackages ?? 1);
      setDirect(c.fullCapacity != null ? String(c.fullCapacity) : "");
      setEditedPackageSize(c.packageSize);
      setSaveAsRecommended(false);
      setSaveAsPackageSize(false);
    } else {
      setConsumeQty("");
      setConsumeMemo("");
    }
  ...
  const refillQuantity =
    mode === "full" ? 1
    : mode === "package"
      ? selected && packages > 0 && editedPackageSize > 0 ? packages * editedPackageSize : 0
      ...
      const body =
        mode === "full"
          ...
          : {
              ...
              memo: mode === "package" ? `${packages}통 × ${editedPackageSize}` : "직접 수량 입력",
              ...(mode === "package" && saveAsRecommended && { recommendedPackages: packages }),
            };
      if (mode === "package" && saveAsPackageSize && editedPackageSize !== selected.packageSize) {
        await apiFetch(`/api/cassettes/${selected.id}`, {
          method: "PATCH",
          body: JSON.stringify({ packageSize: editedPackageSize }),
        });
      }
          ...
          <div className="relative">
            ...
            {results !== null && (
              <div className="absolute left-0 right-0 top-full z-20 mt-1">
                {results.length === 0 ? (
                  <p className="rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm text-slate-400 shadow-lg">검색 결과가 없습니다.</p>
                ) : (
                  <ul className="divide-y divide-slate-100 rounded-lg border border-slate-200 bg-white overflow-hidden max-h-60 overflow-y-auto shadow-lg">
                    {results.map((c, idx) => (
                      <li key={c.id}>
                        <button
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  <div className="flex items-center overflow-hidden rounded-lg border border-slate-300 bg-white">
                    <button
                      type="button"
                      className="px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 transition disabled:opacity-30"
                      onClick={() => setPackages((p) => Math.max(0, p - 1))}
                      disabled={packages <= 0}
                    >−</button>
                    <input
                      className="w-16 bg-white py-2 text-center text-sm font-semibold outline-none"
                      type="number"
                      min={0}
                    ...
                    <button
                      type="button"
                      className="px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 transition"
                      onClick={() => setPackages((p) => p + 1)}
                    >+</button>
                  </div>
                  <span className="text-sm text-slate-600">통 × </span>
                  <div className="flex items-center gap-1">
                    <input
                      className="input w-20 num text-center"
                      type="number"
                      min={1}
                      value={editedPackageSize === 0 ? "" : editedPackageSize}
                      onChange={(e) => {
                        const v = parseInt(e.target.value, 10);
                        setEditedPackageSize(Number.isFinite(v) && v > 0 ? v : 0);
                      }}
                    />
                    <span className="text-sm text-slate-600">정/통</span>
                  </div>
                  {refillQuantity > 0 && (
                    <span className="text-sm text-slate-600">= <b className="text-slate-800">{fmt(refillQuantity)}정</b></span>
                  )}
                </div>
                {editedPackageSize !== selected.packageSize && (
                  <label className="flex items-center gap-2 text-sm text-slate-600 cursor-pointer select-none">
                    <input
                      type="checkbox"
                      checked={saveAsPackageSize}
                      onChange={(e) => setSaveAsPackageSize(e.target.checked)}
                      className="rounded"
                    />
                    변경한 단위({editedPackageSize}정/통)를 이 카세트의 기본 단위로 저장
                  </label>
                )}
              </div>
            ) : mode === "direct" ? (
